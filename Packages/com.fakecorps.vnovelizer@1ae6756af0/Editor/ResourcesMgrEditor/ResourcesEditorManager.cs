...
public class ResourceManagerWindow : EditorWindow
{
    ...
    private static readonly string[] BackgroundExtensions = { ".png", ".jpg", ".jpeg", ".tga", ".webp" };
    ...
    private void ImportFile()
    {
        ...
        string destAssetPath = Path.Combine(targetAssetPath, fileName).Replace('\\', '/');
        // 将 Assets/Resources/... 转换为系统绝对路径
        string projectRoot = Path.GetDirectoryName(Application.dataPath);
        string destPath = Path.Combine(projectRoot, destAssetPath);
        ...
        try
        {
            if (currentType == ResType.Background && !ResolveBackgroundImportConflicts(targetAssetPath, fileName))
            {
                return;
            }
            ...
            VNovelizerAddressablesUtility.SyncResourcesToAddressables();
    ...
    private void RefreshRightPane()
    {
        ...
        if (currentType == ResType.Background)
        {
            RefreshBackgroundPane(path);
            return;
        }
    ...
    private void RefreshBackgroundPane(string path)
    {
        ...
        foreach (string filePath in allFiles)
        {
            ...
            if (!BackgroundExtensions.Contains(extension))
            {
                continue;
            }
    ...
    private string GetExtensionFilter(ResType type)
    {
        switch (type)
        {
            case ResType.Background: return "png,jpg,jpeg,tga,webp";
            case ResType.BGM:
            case ResType.SFX:
    ...
    private bool ResolveBackgroundImportConflicts(string targetAssetPath, string fileName)
    {
        string destExtension = Path.GetExtension(fileName).ToLower();
        string baseName = Path.GetFileNameWithoutExtension(fileName);

        foreach (string extension in BackgroundExtensions)
        {
            ...
            string conflictingAssetPath = Path.Combine(targetAssetPath, baseName + extension).Replace('\\', '/');
            string projectRoot = Path.GetDirectoryName(Application.dataPath);
            string conflictingFullPath = Path.Combine(projectRoot, conflictingAssetPath);

            if (!File.Exists(conflictingFullPath))
            {
                continue;
            }
            ...
            bool replace = EditorUtility.DisplayDialog(
                "同名背景已存在",
                $"已存在同名背景：\n{conflictingAssetPath}\n\n继续导入会替换为：\n{fileName}\n\n这样可以避免 Addressables 中出现同名地址。",
                "替换",
                "取消");
            ...
            AssetDatabase.DeleteAsset(conflictingAssetPath);
        }
