...
public class CharacterEditorWindow : EditorWindow
{
    ...
    private void DrawExpressionList(CharacterProfile profile)
    {
        ...
        }, () => RemoveSelectedListItem(profile.ElementSprites, profile, elementListView));
    ...
    private void DrawHeadList(CharacterProfile profile)
    {
        ...
        }, () => RemoveSelectedListItem(profile.HeadSprites, profile, headSpriteListView));
    ...
    private ListView CreateStyledListView(List<ElementSprite> sourceList, CharacterProfile profile, ListView existingView)
    {
        ...
        listView.fixedItemHeight = 58; // 行高
    ...
    private VisualElement CreateListItem()
    {
        ...
        var delBtn = new Button() { text = "删除", name = "Delete" };
        delBtn.style.width = 46;
        delBtn.style.marginLeft = 5;
        delBtn.style.backgroundColor = new Color(0.45f, 0.18f, 0.18f);
        delBtn.style.color = Color.white;

        var pathField = new TextField("Remote Key") { name = "Path" };
        pathField.style.marginTop = 1;

        topRow.Add(nameField);
        topRow.Add(spriteField);
        topRow.Add(delBtn);
    ...
    private void BindListItem(VisualElement element, int index, List<ElementSprite> list, CharacterProfile profile, ListView listView)
    {
        ...
        var pathField = element.Q<TextField>("Path");
        ...
        pathField.SetValueWithoutNotify(data.SpritePath ?? string.Empty);
        ...
        spriteField.RegisterValueChangedCallback(evt => {
            ...
            data.SpritePath = BuildRemoteAssetKey(data.Sprite);
            pathField.SetValueWithoutNotify(data.SpritePath);
            EditorUtility.SetDirty(profile);
            if (listView.selectedIndex == index) UpdatePreview(data.Sprite);
        });

        pathField.RegisterValueChangedCallback(evt => {
            data.SpritePath = evt.newValue;
            EditorUtility.SetDirty(profile);
        });
        ...
        pathField.RegisterCallback(onFocus);

        delBtn.clicked += () => RemoveListItem(list, profile, listView, index);
    }
    ...
    private void BackfillCharacterSpritePaths()
    {
        bool changedAny = false;
        foreach (CharacterProfile profile in allProfiles)
        {
            if (profile == null)
            {
                continue;
            ...
            bool changed = BackfillSpritePaths(profile.ElementSprites);
            changed |= BackfillSpritePaths(profile.HeadSprites);
            if (changed)
            {
                EditorUtility.SetDirty(profile);
                changedAny = true;
    ...
    private bool BackfillSpritePaths(List<ElementSprite> entries)
    {
        if (entries == null)
        {
            return false;
        ...
        {
            if (entry == null || !string.IsNullOrEmpty(entry.SpritePath))
            {
                continue;
            }

            string spritePath = BuildRemoteAssetKey(entry.Sprite);
    ...
    private static string BuildRemoteAssetKey(Object asset)
    {
        ...
        {
            return StripExtension(assetPath.Substring(resourcesPrefix.Length));
        }

        const string remotePrefix = "Assets/RemoteContent/";
        if (assetPath.StartsWith(remotePrefix))
        {
            return StripExtension(assetPath.Substring(remotePrefix.Length));
        }
    ...
    private void RemoveSelectedListItem(List<ElementSprite> list, CharacterProfile profile, ListView listView)
    {
        if (listView == null || listView.selectedIndex < 0)
        {
            EditorUtility.DisplayDialog("删除条目", "请先在列表中选择一个要删除的条目。", "确定");
            return;
        }

        RemoveListItem(list, profile, listView, listView.selectedIndex);
    }

    private void RemoveListItem(List<ElementSprite> list, CharacterProfile profile, ListView listView, int index)
    {
        if (list == null || listView == null || index < 0 || index >= list.Count)
        {
            return;
        }

        ElementSprite item = list[index];
        string itemName = string.IsNullOrEmpty(item?.Element) ? $"第 {index + 1} 项" : item.Element;
        if (!EditorUtility.DisplayDialog("删除条目", $"确定要删除“{itemName}”吗？", "删除", "取消"))
        {
            return;
        }
    ...
    private VisualElement CreateListHeader(string title, System.Action onAdd, System.Action onRemoveSelected)
    {
        ...
        var removeBtn = new Button(onRemoveSelected) { text = "删除选中" };
        removeBtn.style.marginRight = 5;
        removeBtn.style.backgroundColor = new Color(0.45f, 0.18f, 0.18f);
