...
public class GameStateManager : BaseManager<GameStateManager>
{
    ...
    {
        bool stateChanged = currentState != GameState.Gameplay || previousState != GameState.Gameplay || stateStack.Count > 0;
        ...
        if (stateChanged)
        {
            Debug.Log("[GameState] Reset to Gameplay");
    ...
    public bool IsInPanelState()
    {
        return currentState == GameState.History || 
               ...
               currentState == GameState.Panorama ||
