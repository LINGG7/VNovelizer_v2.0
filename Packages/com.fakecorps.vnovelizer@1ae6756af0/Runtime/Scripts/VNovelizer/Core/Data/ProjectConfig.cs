...
public class VNProjectConfig : ScriptableObject
{
        ...
        get
        {
            if (_instance == null)
            {
                ...
                try
                {
                    if (_instance == null)
                    {
                        var addressablesHandle = Addressables.LoadAssetAsync<VNProjectConfig>("VNProjectConfig");
                        _instance = addressablesHandle.WaitForCompletion();
                    }
                }
                catch (System.Exception e)
                {
                    Debug.LogWarning($"VNProjectConfig Addressables load failed, falling back to Resources: {e.Message}");
                }
#endif
    ...
    [Tooltip("EndingChoice在Resources下的相对路径 (例如: VNovelizerRes/VNPrefabs/UI/EndingChoice)")]
    public string UI_EndingChoicePath = "VNovelizerRes/VNPrefabs/UI/EndingChoice";
    ...
    [Tooltip("结局数据容器在Resources下的相对路径 (例如: VNovelizerRes/GalleryContent/Ending)")]
    public string Ending_DataPath = "VNovelizerRes/GalleryContent/Ending";
