...
public class SceneSlot : MonoBehaviour
{
    ...
    private void UpdateImage()
    {
        if (image == null)
        {
            Debug.LogWarning("[SceneSlot] Image is null, cannot update sprite.");
            return;
        }

        Sprite fallbackSprite = SceneImageLoader.GetFallbackSprite(sceneData, isUnlocked);
        ApplySprite(fallbackSprite);

        string spritePath = SceneImageLoader.GetSpritePath(sceneData, isUnlocked);
        int version = ++spriteLoadVersion;
        if (!string.IsNullOrEmpty(spritePath))
        {
            StartCoroutine(SceneImageLoader.LoadSpriteAsync(sceneData, isUnlocked, sprite =>
            {
