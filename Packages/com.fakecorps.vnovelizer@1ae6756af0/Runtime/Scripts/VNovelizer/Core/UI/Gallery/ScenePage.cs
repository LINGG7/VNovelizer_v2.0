...
public class ScenePage : MonoBehaviour
{
    ...
    private GameObject sceneSlotDetailPrefab;
    private SceneSlotDetail sceneSlotDetail;
    ...
    public void Hide()
    {
        if (sceneSlotDetail != null)
        {
            sceneSlotDetail.Hide();
        }
    ...
    /// 场景上一页
    /// </summary>
    private void ShowSceneSlotDetail(VNScene sceneData)
    {
        EnsureSceneSlotDetail();
        if (sceneSlotDetail != null)
        {
            Sprite fallbackSprite = SceneImageLoader.GetFallbackSprite(sceneData, true);
            sceneSlotDetail.Show(sceneData, fallbackSprite);

            string spritePath = SceneImageLoader.GetSpritePath(sceneData, true);
            int version = ++detailSpriteLoadVersion;
            if (!string.IsNullOrEmpty(spritePath))
            {
                StartCoroutine(SceneImageLoader.LoadSpriteAsync(sceneData, true, sprite =>
                {
                    if (this == null || version != detailSpriteLoadVersion || sceneSlotDetail == null)
                    {
                        return;
                    }

                    sceneSlotDetail.SetSprite(sprite);
                }));
            }
        }
    }

    private void EnsureSceneSlotDetail()
    {
        if (sceneSlotDetail != null)
        {
            return;
        ...
        Transform existing = transform.Find("SceneSlotDetail");
        if (existing != null)
        {
            sceneSlotDetail = existing.GetComponent<SceneSlotDetail>();
            if (sceneSlotDetail == null)
            {
                sceneSlotDetail = existing.gameObject.AddComponent<SceneSlotDetail>();
            }
            return;
        }

        if (sceneSlotDetailPrefab == null)
        {
            string loadPath = VNProjectConfig.Instance.UI_GalleryPath + "/Scene";
            sceneSlotDetailPrefab = ResourcesManager.GetInstance().Load<GameObject>(loadPath + "/SceneSlotDetail");
        }

        if (sceneSlotDetailPrefab == null)
        {
            Debug.LogError("[ScenePage] 场景详情预制体未找到");
            return;
        }

        GameObject detailObj = Instantiate(sceneSlotDetailPrefab, transform);
        detailObj.name = "SceneSlotDetail";
        sceneSlotDetail = detailObj.GetComponent<SceneSlotDetail>();
    }
