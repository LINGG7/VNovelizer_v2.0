...
namespace VNovelizer.Core.Commands
{
    public class ChoiceCommand : VNCommand
    {
        ...
        public override IEnumerator ExecuteAsync(string args)
        {
            ...
            while (gameplayPanel != null && gameplayPanel.IsTextTyping())
            {
                yield return null;
        ...
        public override bool Execute(string args)
        {
            ...
            string selectedFlagKey = BuildSelectedFlagKey(args);
            ...
            if (panel == null || !panel.gameObject.activeSelf)
            {
                ...
                {
                    p.AddChoice(text, cmd, selectedFlagKey);
                });
            }
            else
            {
                // 如果已经开了，直接加按钮
                panel.AddChoice(text, cmd, selectedFlagKey);
            }

            return true;
        }

        private static string BuildSelectedFlagKey(string args)
        {
            string scriptName = VNManager.GetInstance().GetCurrentScriptName();
            string source = $"{scriptName}:{args ?? string.Empty}";
            return $"choice_selected:{scriptName}:{StableHash(source)}";
        }

        private static string StableHash(string source)
        {
