...
public class UIManager : BaseManager<UIManager>
{
    ...
    private readonly HashSet<string> preloadedPanelPrefabKeys = new HashSet<string>();
    ...
    private Transform _preloadInactiveRoot;

    private const string TransitionManagerPath = "VNovelizerRes/VNPrefabs/UI/TransitionManagerRoot";
    ...
    private void StretchLayerReferencesToCanvas()
    {
        StretchLayerToCanvas(Bottom);
        StretchLayerToCanvas(Top);
        StretchLayerToCanvas(Left);
        StretchLayerToCanvas(Middle);
        StretchLayerToCanvas(Right);
        StretchLayerToCanvas(System);
    }

    private void StretchLayerToCanvas(Transform layer)
    {
    ...
    private void EnsureTransitionManager()
    {
        ...
        TransitionManager existingManager =
            Object.FindFirstObjectByType<TransitionManager>(FindObjectsInactive.Include);
        ...
        GameObject transitionRoot =
            ResourcesManager.GetInstance().Load<GameObject>(TransitionManagerPath);
        if (transitionRoot == null)
        {
            Debug.LogError($"[UIManager] 无法加载转场管理器: {TransitionManagerPath}");
            return;
        }

        if (transitionRoot.GetComponent<TransitionManager>() == null)
        {
            Debug.LogError("[UIManager] TransitionManagerRoot 上未找到 TransitionManager 组件。");
            GameObject.Destroy(transitionRoot);
        }
    }
    /// <summary>
    /// 查找主Canvas
    ...
    private void RefreshLayerReferences()
    {
        ...
        StretchLayerReferencesToCanvas();
        EnsureLayerSiblingOrder();
    ...
    private void EnsureLayerSiblingOrder()
    {
        int index = 0;
        SetLayerSibling(Bottom, ref index);
        SetLayerSibling(Left, ref index);
        SetLayerSibling(Middle, ref index);
        SetLayerSibling(Right, ref index);
        SetLayerSibling(Top, ref index);
        SetLayerSibling(System, ref index);
    }

    private void SetLayerSibling(Transform layer, ref int index)
    {
        if (layer == null)
            return;
...
// }
    ...
    public void ShowPanel<T>(
        ...
        UnityAction onFailed = null) where T : BasePanel
    {
        ...
        ResourcesManager.GetInstance().LoadLocalFirstAsync<GameObject>(
            loadPath +"/"+ panelName, 
            (obj) =>
            {
    ...
    public bool ShowPanelWithDarkFade<T>(
        string panelName,
        string loadPath,
        E_UI_Layer layer,
        UnityAction<T> callBack,
        UnityAction onFailed = null) where T : BasePanel
    {
        ...
        if (transitionManager == null ||
            !transitionManager.HasEffect(DarkFadeTransitionEffect.EffectKeyConst))
        {
            Debug.LogWarning("[UIManager] 黑场转场不可用，直接显示面板。");
            ShowPanel(panelName, loadPath, layer, callBack, onFailed);
            return true;
        }

        return transitionManager.PlayDarkFadeTransitionAsync(
            middleActionAsync: finishMiddleAction =>
            {
                ShowPanel<T>(
                    panelName,
                    loadPath,
    ...
    public bool HidePanelWithDarkFade(
        string panelName,
        ...
        UnityAction onComplete = null)
    {
        ...
        if (transitionManager == null ||
            !transitionManager.HasEffect(DarkFadeTransitionEffect.EffectKeyConst))
        {
            Debug.LogWarning("[UIManager] 黑场转场不可用，直接关闭面板。");
            HidePanel(panelName);
            onHidden?.Invoke();
            onComplete?.Invoke();
            return true;
    ...
    public void PreloadPanelPrefab(string panelName, string loadPath)
    {
        if (string.IsNullOrEmpty(panelName) || string.IsNullOrEmpty(loadPath))
        {
            return;
        }
        ...
        string assetKey = loadPath + "/" + panelName;
        if (!preloadedPanelPrefabKeys.Add(assetKey))
        {
            return;
        }
        ...
        if (localPrefab != null)
        {
            CachePreloadedPanelInstance(panelName, assetKey, InstantiateInactivePanel(localPrefab));
            return;
        }
    ...
    private void CachePreloadedPanelInstance(string panelName, string assetKey, GameObject obj)
    {
        if (obj == null)
        {
            VNDebug.LogVerbose($"[UIManager] UI preload skipped: {assetKey}");
            return;
        }
        ...
        Transform father = GetLayerFather(E_UI_Layer.Middle);
        if (father == null)
        {
            Init();
            father = GetLayerFather(E_UI_Layer.Middle);
        }

        if (father == null)
        {
            Debug.LogWarning($"[UIManager] UI preload skipped because Middle layer is missing: {assetKey}");
            GameObject.Destroy(obj);
            return;
        }
        ...
        if (panel == null)
        {
            Debug.LogWarning($"[UIManager] UI preload skipped because BasePanel is missing: {assetKey}");
            GameObject.Destroy(obj);
            return;
        }
        ...
        obj.SetActive(false);
        panelDic.Add(panelName, panel);
        VNDebug.LogVerbose($"[UIManager] UI panel instance preloaded: {assetKey}");
    }

    private GameObject InstantiateInactivePanel(GameObject prefab)
    {
        Transform inactiveRoot = GetPreloadInactiveRoot();
        GameObject obj = GameObject.Instantiate(prefab, inactiveRoot, false);
        obj.SetActive(false);
        return obj;
    }

    private Transform GetPreloadInactiveRoot()
    {
        if (_preloadInactiveRoot != null)
        {
            return _preloadInactiveRoot;
        }

        GameObject root = new GameObject("UIPreloadInactiveRoot");
        root.SetActive(false);
        _preloadInactiveRoot = root.transform;
        return _preloadInactiveRoot;
    }
