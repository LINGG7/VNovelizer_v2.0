...
完整参数、执行时机、并行规则和项目自定义命令见 [CommandReference.md](Docs/CommandReference.md)。

指令不区分大小写，参数间用逗号分隔。多条顶层指令用 `&` 串行连接；需要同时启动多条指令时，使用 `parallel(...)`，并在括号内用 `;` 分隔并行成员。并行组会等待所有成员完成后才继续后面的串行指令。

```text
charmove(M,100,0,1)&bgfade(room,1)
parallel(charmove(M,100,0,1);bgfade(room,1);playsfx(door,1))&wait(0.5)
parallel(wait(0.5);parallel(charmove(L,-300,0,1);charmove(R,300,0,1)))
```

第一行是串行示例；第二行会同时启动移动、背景切换和音效，三者全部完成后才执行 `wait`；第三行展示嵌套并行。

`parallel(...)` 只在剧本的异步流程中执行；通过同步 `ExecuteCommand` API 调用时会被拒绝并记录警告。
...
*   `charscale(scale, duration)`: 缩放当前可见立绘。也可用 `charscale(pos, scale, duration)` 缩放指定槽位。
